...
def get_curve_attributions(data):
    ...
    for i in range(len(data)):
        "取响应阶段初始值"
        init_value = min(data[i][:100])
        "取响应阶段最大值"
        response_value = max(data[i][:300])
        "求响应敏感度"
        attributions_response_sensitivity.append(10 * (response_value - init_value) / init_value)
...
class SerialSvmGui(QtWidgets.QWidget, Ui_Form):
    def __init__(self):
        ...
        self.curve_1 = self.pw.plot(pen='k', name='TGS2600', symbol='o', symbolSize=4, symbolPen=(0, 0, 0),
                                    symbolBrush=(0, 0, 0))
        self.curve_2 = self.pw.plot(pen='r', name='TGS2610', symbol='t', symbolSize=4, symbolPen=(255, 0, 0),
                                    symbolBrush=(255, 0, 0))
        self.curve_3 = self.pw.plot(pen='b', name='TGS2611', symbol='s', symbolSize=4, symbolPen=(0, 0, 255),
                                    symbolBrush=(0, 0, 255))
        self.curve_4 = self.pw.plot(pen='g', name='TGS2620', symbol='d', symbolSize=4, symbolPen=(0, 255, 0),
                                    symbolBrush=(0, 255, 0))
    ...
    def svm_eval(self):
        ...
        try:
            ...
            one_period_attributions = one_period_attributions.reshape(1, 4)
        except:
        ...
        print("预测标签为:{}".format(predicted[0]))
        ...
        sof2_concentration = predicted[0] * 5
        ...
        # SOF2浓度与烧蚀量的关系
        if (sof2_concentration >= 1.19) and (sof2_concentration <= 338.76):
